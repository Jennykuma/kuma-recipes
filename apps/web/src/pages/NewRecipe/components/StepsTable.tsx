...
const StepsTable = ({
  ...
  const { fields, append, insert, remove } = useFieldArray({
    ...
    rules: {
      validate: (items) =>
        items.some((item) => item.step.trim() !== '') || 'At least one step is required',
    },
  });
        ...
        {fields.map((field, index) => {
          return (
            <div key={field.id} className="flex items-center gap-1">
              ...
              <input
                className={classNames(
                  'w-full text-sm pl-1 rounded-md border transition-colors focus:outline-none dark:bg-canvas-deep dark:text-gray-100',
                  activeIndex === index
                ...
                onBlur={handleBlur}
                onKeyDown={(e) =>
                  handleKeyDown(e, stepTableRows?.[index]?.step ?? '', index)
                }
              />
              {fields.length !== 1 && (
                <button
