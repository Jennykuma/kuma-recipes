...
Use the accessibility skill to audit the Kuma Recipes frontend for accessibility issues.

Use the accessibility skill to audit `apps/web/src/pages/NewRecipe/NewRecipe.tsx` for semantic HTML, labels, keyboard access, focus states, ARIA issues, screen reader clarity, and form error accessibility. Report findings by severity with file references and suggested fixes.

Use the accessibility skill to review the recipe creation and recipe details flows in `apps/web` for accessibility regressions. Focus on keyboard navigation, labels, focus visibility, screen reader support, and contrast.

Use the accessibility skill to review this branch’s frontend changes for accessibility issues. List findings first by severity, include file references, and suggest the smallest clear fix for each issue.
