...
All API input boundaries use Zod schemas for runtime validation. The schema is the canonical source of truth; TypeScript types are derived from it via `z.infer<>`. Schemas are named `<Concept>Schema` (e.g. `NewRecipeBodySchema`, `VariantItemSchema`).

Canonical types and schemas live in `packages/shared/src/` and are imported by both apps via `'shared'`. API `*.types.ts` files are thin re-exports — new types go in `packages/shared/src/`, not in the API directly.
